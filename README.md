...
- 自動分段：每 15 / 30 / 60 / 120 分鐘，或者每 100 MB / 250 MB / 500 MB / 1 GB 開新檔
- WAV 一定會喺接近 4 GB 之前自動分段（RIFF 個長度欄位淨係 32-bit，過咗就寫出壞檔案）。
  96kHz / 24-bit / 立體聲大約兩個鐘就到
- VOX 聲控：靜音嗰陣唔寫入，一有聲即刻繼續（門檻 -50 至 -30 dB 可調，電平錶上面睇得到條門檻線）
