...
- ✅ P1 記憶播放位置 —— 每 5 秒同離開畫面時寫入，返嚟自動續播（頭尾 3 秒內唔算）
- ✅ P1 睡眠計時器（5 / 15 / 30 / 60 分鐘，會顯示倒數）
- ✅ P1 播放時加書籤（Bookmark here）／長按書籤刪除
- ✅ P1 來電自動暫停 —— 用 `AudioManager.OnModeChangedListener`（API 31+），
  唔使 `READ_PHONE_STATE` 權限。收線之後自動繼續；如果係你手動撳暫停就唔會自動續
- ✅ P1 搜尋清除掣
- ✅ P2 設定頁加咗 About（版本號同 build 號）

順帶：Room 由 v1 升到 v2 加 `lastPositionMs`，寫咗真嘅 migration 而唔係
destructive fallback —— 後者會連我的最愛、備註、書籤一齊清走，嗰啲數據冇第二個
地方有。

未做（下一批候選）：A-B 循環、音量增益、錄音中睇到書籤位置、自動分段、VOX、
分割／合併／淡化、SAF 自訂儲存位置、匯出細碼率。
